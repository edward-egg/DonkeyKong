...
<html>
    ...
    <script>
                ...
                success: function (list) {
                    ...
                    $.each(list, function (index, user) {
                        var str = "<tr id=" + user.userId + "><td>"
                                ...
                                + "<button type='button' class='edit' id='editUser' onclick=\"editUser('" + user.userId + "','" + user.email + "','" + user.tel + "','" + user.level + "','" + user.freeze + "')\">Edit</button>"
            ...
            $('#edit-dialog').dialog({
                ...
                buttons: {
                    "Edit": function () {
                        ...
                        all = $([]).add(email).add(tel).add(pass).add(level);
                        var bValid = true;
                        all.removeClass("ui-state-error");
                        bValid = bValid && checkLength(id, "ID", 0, 16);
                        bValid = bValid && checkLength(pass, "password", 0, 16);
                        bValid = bValid && checkLength(level, "level", 0, 3);
                        if (tel.val() != "") {
                            bValid = bValid && checkRegexp(tel, /^1[34578]\d{9}$/, "Invalid Tel");
                        }
                        if(email.val() != ""){
                        ...
                        if (bValid) {
                            ...
                            $.ajax({
                                ...
                                type: 'POST',
                                data: {
                                    "userId": $('#e-uid').val() == '' ? userId : $('#e-uid').val(),
                                    "password": $('#e-upass').val() == '' ? userpassword : $('#e-upass').val(),
                                    "email": $('#e-email').val() == '' ? useremail : $('#e-email').val(),
                                    "level": $('#e-level').val() == '' ? userlevel : $('#e-level').val(),
                                    "tel": $('#e-tel').val() == '' ? usertel : $('#e-tel').val(),
                                    "freeze": $('#e-freeze').val() == '' ? userfreeze : $('#e-freeze').val()
                                },
                                success: function () {
        ...
        function editUser(uid, upass, umail, utel, ulevel, ufreeze) {
            userId = uid;
            userpassword = upass;
            usermail = umail;
            usertel = utel;
